...
{{cookiecutter.git_repo_name}} is an [AWS Lambda in Go](https://github.com/aws/aws-lambda-go) binded to an API Gateway which returns the value of the environment variable `ECHO_MESSAGE` on a `GET /echo` request.

The [3 Musketeers](https://github.com/flemay/3musketeers) pattern is being used to test, build, and deploy the Lambda function. [Serverless](https://serverless.com) is the chosen framework to handle AWS deployment.
